...
void create_node(struct node *find_sr, int quantity)
{
    ...
    //! Inserting in the treap
    char arr[100];
    treap *p = NULL;
    strcpy(arr, find_sr->foodname);

    if (t == NULL)
    {
        t = insert_treap(t, arr, p, quantity);
    }
    else
    {
        t = insert_treap(t, arr, p, quantity);
